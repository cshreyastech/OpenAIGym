...
class QNAgent(Agent):
  ...
  def build_model(self):
    ...
    self.state_in = tf.placeholder(tf.int32, shape=[1]) # (1, )
    self.action_in = tf.placeholder(tf.int32, shape=[1]) # (1, )
    self.target_in = tf.placeholder(tf.float32, shape=[1]) # (1, )

    self.state = tf.one_hot(self.state_in, depth=self.state_size) # (1, state_size)
    self.action = tf.one_hot(self.action_in, depth=self.action_size) # (1, action_size)
